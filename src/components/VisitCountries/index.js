...
class VisitCountries extends Component {
  ...
  onClickRemoveCountry = countryId => {
    const {countriesList} = this.state

    const newCountryList = countriesList.map(eachCountry => {
      if (eachCountry.id === countryId) {
        const removedItem = {
          ...eachCountry,
          isVisited: !eachCountry.isVisited,
    ...
    this.setState({countriesList: newCountryList})
  }
      ...
      <MainContainer>
        ...
        <AllCountriesTable>
          {countriesList.map(eachCountry => (
            <CountryData
              ...
              changeCountryVisitStatus={this.onClickVisitButton}
            />
          ))}
        </AllCountriesTable>
        ...
        <VisitCountriesList>
          {visitedCountriesList.length === 0 ? (
            <NoCountriesView>No Countries Visited Yet</NoCountriesView>
          ) : (
            visitedCountriesList.map(
              eachCountry =>
                eachCountry.isVisited && (
                  <VisitedCountry
                    key={eachCountry.id}
                    visitedCountryDetails={eachCountry}
                    removeCountryFromVisitedList={this.onClickRemoveCountry}
                  />
                ),
            )
          )}
